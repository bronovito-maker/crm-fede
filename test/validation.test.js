...
function throwsWithCode(fn, expectedCode) {
  assert.throws(fn, (err) => {
    assert.equal(err.code, expectedCode, `Atteso codice errore: ${expectedCode}, ricevuto: ${err.code}`);
    return true;
  });
}
...
describe("sanitizeContractInput", () => {
  describe("contratto valido", () => {
    ...
    it("normalizza IBAN rimuovendo spazi e portando a maiuscolo", () => {
      const result = sanitizeContractInput({ ...validContract, iban: "it60 x054 2811 1010 0000 0123 456" });
    ...
    it("accetta bollettino senza IBAN", () => {
      const result = sanitizeContractInput({ ...validContract, metodoPagamento: "bollettino", iban: "" });
  ...
  describe("campi obbligatori", () => {
    it("lancia CUSTOMER_REQUIRED se ragione sociale vuota", () => {
      throwsWithCode(() => sanitizeContractInput({ ...validContract, ragioneSociale: "  " }), "CUSTOMER_REQUIRED");
    });

    it("lancia PHONE_REQUIRED se cellulare vuoto", () => {
      throwsWithCode(() => sanitizeContractInput({ ...validContract, cellulare: "" }), "PHONE_REQUIRED");
    });

    it("lancia SUPPLIER_REQUIRED se fornitore vuoto", () => {
      throwsWithCode(() => sanitizeContractInput({ ...validContract, fornitore: "" }), "SUPPLIER_REQUIRED");
    });

    it("lancia OFFER_REQUIRED se nome offerta vuoto", () => {
      throwsWithCode(() => sanitizeContractInput({ ...validContract, nomeOfferta: "" }), "OFFER_REQUIRED");
    });
  });

  describe("valori con whitelist", () => {
    it("lancia CLIENT_TYPE_INVALID per tipo cliente non previsto", () => {
      throwsWithCode(() => sanitizeContractInput({ ...validContract, tipoCliente: "Azienda" }), "CLIENT_TYPE_INVALID");
    });

    it("lancia CUSTOMER_CATEGORY_INVALID per categoria non prevista", () => {
      throwsWithCode(() => sanitizeContractInput({ ...validContract, categoriaCliente: "vip" }), "CUSTOMER_CATEGORY_INVALID");
    });

    it("lancia OPERATION_INVALID per tipo operazione non previsto", () => {
      throwsWithCode(() => sanitizeContractInput({ ...validContract, tipoOperazione: ["hackeraggio"] }), "OPERATION_INVALID");
    });

    it("lancia OPERATION_INVALID se tipoOperazione è array vuoto", () => {
      throwsWithCode(() => sanitizeContractInput({ ...validContract, tipoOperazione: [] }), "OPERATION_INVALID");
    });
  ...
  describe("validazione condizionale fornitura", () => {
    it("lancia POD_REQUIRED se tipo fornitura è luce e POD mancante", () => {
      throwsWithCode(() => sanitizeContractInput({ ...validContract, tipoFornitura: "luce", pod: "" }), "POD_REQUIRED");
    });

    it("lancia POD_REQUIRED se tipo fornitura è dual e POD mancante", () => {
      throwsWithCode(
        () => sanitizeContractInput({ ...validContract, tipoFornitura: "dual", pod: "", pdr: "04000000001" }),
        "POD_REQUIRED"
      );
    });
    ...
    it("lancia PDR_REQUIRED se tipo fornitura è gas e PDR mancante", () => {
      throwsWithCode(
        () => sanitizeContractInput({ ...validContract, tipoFornitura: "gas", pod: "", pdr: "", metodoPagamento: "bollettino", iban: "" }),
        "PDR_REQUIRED"
      );
    });
    ...
    it("lancia PDR_REQUIRED se tipo fornitura è dual e PDR mancante", () => {
      throwsWithCode(
        () => sanitizeContractInput({ ...validContract, tipoFornitura: "dual", pod: "IT001E12345678", pdr: "" }),
        "PDR_REQUIRED"
      );
    });
  });
  ...
  describe("validazione formato", () => {
    it("lancia EMAIL_INVALID per email malformata", () => {
      throwsWithCode(() => sanitizeContractInput({ ...validContract, email: "non-una-email" }), "EMAIL_INVALID");
    });
    ...
    it("lancia PIVA_INVALID se P.IVA non ha 11 cifre", () => {
      throwsWithCode(() => sanitizeContractInput({ ...validContract, piva: "123" }), "PIVA_INVALID");
    });

    it("lancia PIVA_INVALID se P.IVA contiene lettere", () => {
      throwsWithCode(() => sanitizeContractInput({ ...validContract, piva: "ABCDE123456" }), "PIVA_INVALID");
    });
...
describe("sanitizeAgentInput", () => {
  describe("agente valido", () => {
    ...
    it("normalizza email a minuscolo", () => {
      const result = sanitizeAgentInput({ ...validAgent, email: "MARCO@EXAMPLE.IT" }, { requirePassword: true });
      assert.equal(result.email, "marco@example.it");
    });

    it("normalizza ruolo a minuscolo", () => {
      const result = sanitizeAgentInput({ ...validAgent, ruolo: "ADMIN" }, { requirePassword: true });
    ...
    it("non richiede password se requirePassword=false e campo vuoto", () => {
      const result = sanitizeAgentInput({ ...validAgent, password: "" }, { requirePassword: false });
      assert.equal(result.nome, "Marco Bianchi");
    });
  });
  ...
  describe("validazione password", () => {
    ...
    it("accetta password di 8+ caratteri", () => {
      assert.doesNotThrow(() => sanitizeAgentInput({ ...validAgent, password: "12345678" }, { requirePassword: true }));
    });
  });
});
